...
double Force(int ip, int idir) {  // Compute forces as -Grad_ip V(r)
  ...
  for (int i = 0; i < npart; ++i) {
    if (i != ip) {
      ...
      if (dr < rcut) {
        f += dvec[idir] *
             (48.0 / pow(dr, 14) - 24.0 / pow(dr, 8));  // -Grad_ip V(r)
      }
    }
  }
...
void Measure() {  // Properties measurement
  ...
  for (int i = 0; i < npart - 1; ++i) {
    for (int j = i + 1; j < npart; ++j) {
      dx =
          Pbc(xold[i] - xold[j]);  // here I use old configurations [old = r(t)]
      dy =
          Pbc(yold[i] - yold[j]);  // to be compatible with EKin which uses v(t)
...
double Pbc(
    double r) {  // Algorithm for periodic boundary conditions with side L=box
