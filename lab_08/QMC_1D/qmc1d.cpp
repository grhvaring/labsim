...
double variationalWaveFunction(double v) {
  double mu = 0.812727, sigma = 0.633333;
  double plus = (v + mu) / sigma;
  double minus = (v - mu) / sigma;
  return exp(-0.5 * minus * minus) + exp(-0.5 * plus * plus);
  // return 1.0;
  // return exp(-0.5*v*v);
}
...
double variationalWaveFunction_second(double v) {
  ...
  double plus = (v + mu) / sigma;
  double minus = (v - mu) / sigma;
  return -minus * exp(-0.5 * minus * minus) - plus * exp(-0.5 * plus * plus);
  // return 0;
